...
export default function TopicsPage() {
  ...
  const [addTopicRows, setAddTopicRows] = React.useState<
    { chapterId: string | null; topicNames: string }[]
  ...
  const [quickCreateSelectedChapterIds, setQuickCreateSelectedChapterIds] =
    React.useState<string[]>([]);
  const [quickCreateTopicsByChapterId, setQuickCreateTopicsByChapterId] =
  ...
  const quickCreateSelectedChaptersSorted = React.useMemo(() => {
    ...
    quickCreateAvailableChapters,
    quickCreateSelectedChapterIds,
    units,
    subjects,
  ]);
  ...
  const quickCreateUnitsGrouped = React.useMemo(() => {
    const byUnit = new Map<
      string,
      { unitId: string; label: string; chapterIds: string[] }
    >();
  ...
  const handleAddTopics = async () => {
    ...
    addTopicRows.forEach((row) => {
      ...
      const names = row.topicNames
        ...
        .filter(Boolean);
      names.forEach((name) =>
        toCreate.push({ chapterId: row.chapterId!, name }),
      );
    });
    ...
    if (duplicate && !created)
      toast.warning(`${duplicate} skipped (already exist in this chapter)`);
  ...
  const toggleQuickCreateByUnit = (unitId: string) => {
    ...
    setQuickCreateSelectedChapterIds((prev) => {
      ...
      if (allSelected)
        return prev.filter((id) => !group.chapterIds.includes(id));
  ...
  const confirmDeleteTopic = async () => {
    ...
    try {
      const res = await fetch(`${TOPICS_API}/${topicToDelete.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
  ...
  const handleToggleStatus = async (topicId: string) => {
    ...
    try {
      ...
      setTopics((prev) =>
        prev.map((t) => (t.id === topicId ? { ...t, status: nextStatus } : t)),
      );
    } catch {
  ...
  const handlePublish = async (
    ...
    try {
      ...
      setTopics((prev) =>
        prev.map((t) =>
          t.id === updated.id ? { ...t, seo: updated.seo ?? t.seo } : t,
        ),
      );
      toast.success(
        noIndex
  ...
  const filteredTopics = React.useMemo(() => {
    return topics.filter((t) => {
      ...
      const matchesChapter =
        chapterFilter === "all" || String(t.chapterId) === chapterFilter;
      const metaFilled = !!(
  ...
  const canAddTopics = addTopicRows.some(
    (r) =>
      r.chapterId &&
      r.topicNames
        .trim()
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Select exam, subject, and unit. Then select chapter(s) and
                  enter topic names for each chapter (one per line).
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={addAnotherChapterRow}
                    >
                      Add Another Chapter
                    </Button>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Select chapters and enter topics for each chapter
                    separately. Each chapter has its own textarea.
                  </p>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <Select
                            value={row.chapterId ?? ""}
                            onValueChange={(v) =>
                              updateAddRow(index, "chapterId", v || null)
                            }
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            updateAddRow(index, "topicNames", e.target.value)
                          }
        ...
        {isQuickCreateOpen ? (
          <Card className="border-border/80 shadow-sm">
            <CardHeader className="space-y-1">
              ...
              <CardDescription>
                Select one or more chapters and enter topic names for each
                chapter (one per line). Content and meta can be added later.
              </CardDescription>
            </CardHeader>
            <Separator />
                ...
                <div className="grid gap-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      >
                        {quickCreateSelectedChapterIds.length ===
                        quickCreateAvailableChapters.length
                          ? "Clear all"
                          : "Select all"}
                      </Button>
                      {quickCreateUnitsGrouped.length > 1 && (
                        <>
                          ...
                          {quickCreateUnitsGrouped.map(
                            ({ unitId, label, chapterIds }) => {
                              ...
                              return (
                                <Button
                                  ...
                                  onClick={() =>
                                    toggleQuickCreateByUnit(unitId)
                                  }
                                >
                                  {allIn ? "✓ " : ""}
                                  {label}
                  ...
                  <div className="rounded-md border bg-background">
                      ...
                      {quickCreateAvailableChapters.map((chapter) => {
                        ...
                        const label =
                          subject && unit
                            ...
                            : chapter.name;
                        const checked = quickCreateSelectedChapterIds.includes(
                          chapter.id,
                        );
                        return (
                          <label
                          ...
                          >
                            <input
                              ...
                              checked={checked}
                              onChange={() =>
                                toggleQuickCreateChapter(chapter.id)
                              }
                              className="h-4 w-4"
                            />
                    ...
                    <div className="border-t px-3 py-2 text-xs text-muted-foreground">
                      ...
                      {quickCreateSelectedChapterIds.length === 1
                        ? ""
                        : "s"}{" "}
                      selected
                    </div>
                      ...
                      {quickCreateSelectedChaptersSorted.map((chapter) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Textarea
                              value={
                                quickCreateTopicsByChapterId[chapter.id] ?? ""
                              }
                              onChange={(e) =>
              ...
              <div className="mt-6 flex flex-wrap gap-2 border-t pt-4">
                ...
                <Button
                  ...
                  disabled={
                    ...
                    quickCreateSelectedChaptersSorted.every(
                      (c) => !(quickCreateTopicsByChapterId[c.id] || "").trim(),
                    )
                  }
                >
                  Create Topics
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1.5">
                    ...
                    <CardDescription className="text-sm text-muted-foreground">
                      Manage and organize your topics, create new topics, and
                      track topic performance across your educational platform.
                    </CardDescription>
                  </div>
                  ...
                  {groupedPaged.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/60 bg-muted/20 py-16 px-6 text-center">
                      ...
                      <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                        Add topics via Add New Topics or Quick Create for
                    ...
                    groupedPaged.map(
                      ({
                        ...
                        <Card
                          key={`${examId}-${subjectId}-${unitId}-${chapterId}`}
                                ...
                                <TableBody>
                                  {groupTopics.map((topic) => (
                                    <TableRow
                                      ...
                                      onDragStart={(e) =>
                                        handleTopicDragStart(e, topic)
                                      }
                                      onDragOver={handleTopicDragOver}
                                      onDragEnter={() =>
                                        handleTopicDragEnter(topic)
                                      }
                                    ...
                                    >
                                      ...
                                      <TableCell className="text-muted-foreground text-xs">
                                        {topic.hasContent && topic.lastModified
                                          ? topic.lastModified
                                          : "—"}
                                      </TableCell>
                                      <TableCell>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                      ...
                      <span>
                        {filteredTopics.length === 0
                          ? 0
                          : `${startIndex + 1}-${Math.min(startIndex + pageSize, filteredTopics.length)} of ${filteredTopics.length}`}
                      </span>
                    </div>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  id="edit-topic-order"
                  type="number"
                  value={editingTopic.orderNumber}
                  onChange={(e) =>
                    setEditingTopic((prev) =>
                      prev
                        ? {
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {topicToDelete?.name}&quot;? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button variant="destructive" onClick={confirmDeleteTopic}>
              Delete Topic
            </Button>
          </DialogFooter>
        </DialogContent>
