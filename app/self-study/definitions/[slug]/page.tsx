...
const API_BASE = "/api/definitions"
...
export default function DefinitionSlugPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  const [definitionId, setDefinitionId] = React.useState<string | null>(null)
  const [definition, setDefinition] = React.useState<Definition | null>(null)
  ...
  React.useEffect(() => {
    params.then((p) => setDefinitionId(p.slug))
  }, [params])
  ...
  React.useEffect(() => {
    ...
    const html = typeof definition.contentBody === "string" ? definition.contentBody : ""
  ...
  }, [definition?.id, definition?.contentBody])

  React.useEffect(() => {
    if (!definitionId) return
    let cancelled = false
    setLoaded(false)
    fetch(`${API_BASE}/${definitionId}`)
      .then((res) => {
        if (cancelled) return
        if (!res.ok) {
          setDefinition(null)
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbLink href="/self-study/definitions">Definitions</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
    ...
    return (
      <div className="flex min-h-0 flex-1 min-w-0 flex-col">
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/definitions">Definitions</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Definition</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </header>
        <div className="flex flex-1 flex-col gap-4 p-6">
          <Card>
            <CardHeader>
              <CardTitle>Definition not found</CardTitle>
              <CardDescription>No definition matches this link. It may have been removed or the URL is incorrect.</CardDescription>
            </CardHeader>
            <CardContent>
              <Button asChild variant="outline">
                <Link href="/self-study/definitions">Back to Definitions</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/definitions">Definitions</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>{capitalize(definition.name)}</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
        <div className="flex items-center gap-3">
