...
function uniqueNamesPerSubTopic(
  names: string[],
...
export default function DefinitionsPage() {
  ...
  const [addDefinitionRows, setAddDefinitionRows] = React.useState<
    { subtopicId: string | null; definitionNames: string }[]
  >([{ subtopicId: null, definitionNames: "" }]);

  const [editingDefinition, setEditingDefinition] =
    React.useState<Definition | null>(null);
  ...
  const [draggedDefinition, setDraggedDefinition] =
    React.useState<Definition | null>(null);
  const [dragOverDefinition, setDragOverDefinition] =
    React.useState<Definition | null>(null);

  const [quickCreateSelectedSubTopicIds, setQuickCreateSelectedSubTopicIds] =
    React.useState<string[]>([]);
  const [
    quickCreateDefinitionsBySubTopicId,
      ...
      setSubTopics(
        list.map(
          (st: {
            ...
            topicId: st.topicId,
            name: st.name,
            orderNumber: st.order ?? 0,
            status: st.status ?? "Active",
          }),
        ),
      );
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to load subtopics");
    }
  }, []);
  ...
  const subjectsForSelectedExam = React.useMemo(
    () =>
      subjects.filter(
        (s) => s.examId === selectedExamId && s.status !== "Inactive",
      ),
  ...
  const unitsForSelectedSubject = React.useMemo(
    () =>
      units.filter(
        (u) => u.subjectId === selectedSubjectId && u.status !== "Inactive",
      ),
  ...
  const chaptersForSelectedUnit = React.useMemo(
    () =>
      chapters
        .filter((c) => c.unitId === selectedUnitId && c.status !== "Inactive")
        .sort((a, b) => a.orderNumber - b.orderNumber),
    [chapters, selectedUnitId],
  );
  const topicsForSelectedChapter = React.useMemo(
    () =>
      topics
        .filter(
          (t) => t.chapterId === selectedChapterId && t.status !== "Inactive",
        )
        .sort((a, b) => a.orderNumber - b.orderNumber),
    [topics, selectedChapterId],
  );
  const subTopicsForSelectedTopic = React.useMemo(
    () =>
      subTopics
        .filter(
          (st) => st.topicId === selectedTopicId && st.status !== "Inactive",
        )
        .sort((a, b) => a.orderNumber - b.orderNumber),
  ...
  const subjectsForExamFilter = React.useMemo(
    () =>
      examFilter === "all"
        ? []
        : subjects.filter((s) => String(s.examId) === examFilter),
    [subjects, examFilter],
  );
  const unitsForSubjectFilter = React.useMemo(
    () =>
      subjectFilter === "all"
        ? []
        : units.filter((u) => String(u.subjectId) === subjectFilter),
    [units, subjectFilter],
  );
  const chaptersForUnitFilter = React.useMemo(
    () =>
      unitFilter === "all"
        ? []
        : chapters
            .filter((c) => String(c.unitId) === unitFilter)
            .sort((a, b) => a.orderNumber - b.orderNumber),
    [chapters, unitFilter],
  );
  const topicsForChapterFilter = React.useMemo(
    () =>
      chapterFilter === "all"
        ? []
        : topics
            .filter((t) => String(t.chapterId) === chapterFilter)
            .sort((a, b) => a.orderNumber - b.orderNumber),
    [topics, chapterFilter],
  );
  const subTopicsForTopicFilter = React.useMemo(
    () =>
      topicFilter === "all"
        ? []
        : subTopics
            .filter((st) => String(st.topicId) === topicFilter)
            .sort((a, b) => a.orderNumber - b.orderNumber),
  ...
  const quickCreateAvailableSubTopics = React.useMemo(() => {
    return subTopics.filter((st) => {
      ...
      return (
        subject &&
        availableExams.some((e) => e.id === subject.examId) &&
        st.status !== "Inactive"
      );
    });
  ...
  const getExamName = (examId: string) =>
    capitalize(exams.find((e) => e.id === examId)?.name ?? "-");
  const getSubjectName = (subjectId: string) =>
    capitalize(subjects.find((s) => s.id === subjectId)?.name ?? "-");
  const getUnitName = (unitId: string) =>
    capitalize(units.find((u) => u.id === unitId)?.name ?? "-");
  const getChapterName = (chapterId: string) =>
    capitalize(chapters.find((c) => c.id === chapterId)?.name ?? "-");
  const getTopicName = (topicId: string) =>
    capitalize(topics.find((t) => t.id === topicId)?.name ?? "-");
  const getSubTopicName = (subTopicId: string) =>
    capitalize(subTopics.find((st) => st.id === subTopicId)?.name ?? "-");
  ...
  const quickCreateSelectedSubTopicsSorted = React.useMemo(() => {
    ...
    quickCreateAvailableSubTopics,
    quickCreateSelectedSubTopicIds,
    topics,
    chapters,
    units,
    subjects,
  ...
  const quickCreateTopicsGrouped = React.useMemo(() => {
    const byTopic = new Map<
      string,
      { topicId: string; label: string; subTopicIds: string[] }
    >();
    quickCreateAvailableSubTopics.forEach((st) => {
      ...
      const subject = unit
        ? subjects.find((s) => s.id === unit.subjectId)
        : null;
      if (!subject) return;
  ...
  const updateAddRow = (
    index: number,
    field: "subtopicId" | "definitionNames",
    value: string | null,
  ) => {
    setAddDefinitionRows((prev) => {
      const next = [...prev];
      if (field === "subtopicId")
        next[index] = { ...next[index], subtopicId: value };
      else next[index] = { ...next[index], definitionNames: value ?? "" };
      return next;
    });
  };
  const removeAddRow = (index: number) =>
    setAddDefinitionRows((prev) => prev.filter((_, i) => i !== index));
  const addAnotherSubTopicRow = () =>
    setAddDefinitionRows((prev) => [
      ...prev,
      { subtopicId: null, definitionNames: "" },
    ]);
  ...
  const handleAddDefinitions = async () => {
    ...
    addDefinitionRows.forEach((row) => {
      ...
      const rawNames = row.definitionNames
        .split(/\r?\n/)
        .map((s) => s.trim())
        .filter(Boolean);
      const existing = new Set(
        definitions
          .filter((d) => d.subtopicId === row.subtopicId)
          .map((d) => d.name.toLowerCase()),
      );
      const names = uniqueNamesPerSubTopic(rawNames, existing);
      names.forEach((name) =>
        toCreate.push({ subtopicId: row.subtopicId!, name }),
      );
    });
    ...
    if (duplicate && !created)
      toast.warning(`${duplicate} skipped (already exist in this subtopic)`);
  ...
  const toggleQuickCreateSubTopic = (id: string) =>
    setQuickCreateSelectedSubTopicIds((prev) =>
      prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id],
    );
  const toggleQuickCreateSelectAll = () =>
    setQuickCreateSelectedSubTopicIds((prev) =>
      prev.length === quickCreateAvailableSubTopics.length
        ? []
        : quickCreateAvailableSubTopics.map((st) => st.id),
    );
  const toggleQuickCreateByTopic = (topicId: string) => {
    ...
    setQuickCreateSelectedSubTopicIds((prev) => {
      ...
      if (allSelected)
        return prev.filter((id) => !group.subTopicIds.includes(id));
  ...
  const handleBulkCreateDefinitions = async () => {
    ...
    quickCreateSelectedSubTopicsSorted.forEach((st) => {
      const raw = (quickCreateDefinitionsBySubTopicId[st.id] || "")
        .split(/\r?\n/)
        .map((s) => s.trim())
        .filter(Boolean);
      const existing = new Set(
  ...
  const openEditDialog = (d: Definition) => {
    ...
    try {
      const res = await fetch(`${DEFINITIONS_API}/${editingDefinition.id}`, {
        ...
        body: JSON.stringify({
          ...
          orderNumber: editingDefinition.orderNumber,
        }),
      });
      if (res.status === 409) {
        toast.warning(
          "A definition with this name already exists in this subtopic",
        );
        return;
      }
      if (!res.ok) {
  ...
  const handleDeleteDefinition = (d: Definition) => {
    setDefinitionToDelete(d);
    setIsDeleteDialogOpen(true);
  };
  const confirmDeleteDefinition = async () => {
    if (!definitionToDelete) return;
    try {
      const res = await fetch(`${DEFINITIONS_API}/${definitionToDelete.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
  ...
  const handlePublish = async (
    ...
    try {
      ...
      setDefinitions((prev) =>
        prev.map((x) =>
          x.id === updated.id ? { ...x, seo: updated.seo ?? x.seo } : x,
        ),
      );
      toast.success(
        noIndex
  ...
  const saveReorderedDefinitions = async () => {
    ...
    const group = definitions
      .filter((d) => d.subtopicId === draggedDefinition.subtopicId)
      .slice()
      .sort((a, b) => a.orderNumber - b.orderNumber);
    const order = group.map((d, idx) => ({ id: d.id, orderNumber: idx + 1 }));
    try {
      const res = await fetch(`${DEFINITIONS_API}/reorder`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDefinitionDragOver = (e: React.DragEvent) => {
    e.preventDefault();
  ...
  const handleDefinitionDrop = (e: React.DragEvent, target: Definition) => {
    ...
    if (
      !isReorderingEnabled ||
      !draggedDefinition ||
      draggedDefinition.id === target.id
    )
      return;
    ...
    setDefinitions((prev) => {
      ...
      const group = prev
        .filter((d) => d.subtopicId === target.subtopicId)
        .slice()
        .sort((a, b) => a.orderNumber - b.orderNumber);
  ...
  const filteredDefinitions = React.useMemo(() => {
    return definitions.filter((d) => {
      ...
      const matchesExam =
        examFilter === "all" || String(subject.examId) === examFilter;
      const matchesSubject =
        subjectFilter === "all" || String(unit.subjectId) === subjectFilter;
      const matchesUnit =
        unitFilter === "all" || String(chapter.unitId) === unitFilter;
      const matchesChapter =
        chapterFilter === "all" || String(topic.chapterId) === chapterFilter;
      const matchesTopic =
        topicFilter === "all" || String(st.topicId) === topicFilter;
      const matchesSubTopic =
        subTopicFilter === "all" || String(d.subtopicId) === subTopicFilter;
      const metaFilled = !!(
  ...
  const pagedDefinitions = filteredDefinitions.slice(
    startIndex,
    startIndex + pageSize,
  );
  ...
  const groupedPaged = React.useMemo(() => {
    ...
    map.forEach((v) => {
      v.definitions.sort((a, b) => a.orderNumber - b.orderNumber);
    });
  ...
  const canAddDefinitions = addDefinitionRows.some(
    (r) =>
      r.subtopicId &&
      r.definitionNames
        .trim()
      ...
      <header className="flex h-14 shrink-0 items-center justify-between gap-4 border-b border-border/60 bg-background/95 px-4 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="flex items-center gap-2">
          ...
          <Separator
            ...
            className="mr-2 h-5 data-[orientation=vertical]:h-4"
          />
          <Breadcrumb>
            <BreadcrumbList>
              <BreadcrumbItem>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Definitions</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Select exam, subject, unit, chapter, and topic. Then select
                  subtopic(s) and enter definition names for each subtopic (one
                  per line). Each definition name must be unique within the same
                  subtopic.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setSelectedTopicId(v || null)}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={addAnotherSubTopicRow}
                    >
                      Add Another SubTopic
                    </Button>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Select subtopics and enter definitions for each subtopic
                    separately. Each subtopic has its own textarea. Each
                    definition name must be unique within the same subtopic.
                  </p>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <Select
                            ...
                            onValueChange={(v) =>
                              updateAddRow(index, "subtopicId", v || null)
                            }
        ...
        {loading ? (
          ...
          <Card className="border-border/80 shadow-sm">
            <CardHeader className="space-y-1">
              ...
              <CardDescription>
                Select one or more subtopics and enter definition names for each
                subtopic (one per line). Duplicate names within a subtopic are
                skipped.
              </CardDescription>
            </CardHeader>
            <Separator />
            <CardContent className="pt-6">
                ...
                <div className="grid gap-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      >
                        {quickCreateSelectedSubTopicIds.length ===
                        quickCreateAvailableSubTopics.length
                          ? "Clear all"
                          : "Select all"}
                      </Button>
                      {quickCreateTopicsGrouped.length > 1 && (
                        <>
                          ...
                          {quickCreateTopicsGrouped.map(
                            ({ topicId, label, subTopicIds }) => {
                              const allIn = subTopicIds.every((id) =>
                                quickCreateSelectedSubTopicIds.includes(id),
                              );
                              return (
                                <Button
                                  key={topicId}
                                  ...
                                  onClick={() =>
                                    toggleQuickCreateByTopic(topicId)
                                  }
                                >
                                  {allIn ? "✓ " : ""}
                                  {label}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Click a topic to select or clear all its subtopics at once.
                  </p>
                  <div className="rounded-md border bg-background">
                    <div className="max-h-56 overflow-auto p-2">
                      {quickCreateAvailableSubTopics.map((st) => {
                        const topic = topics.find((t) => t.id === st.topicId);
                        const chapter = topic
                          ? chapters.find((c) => c.id === topic.chapterId)
                          : null;
                        const unit = chapter
                          ? units.find((u) => u.id === chapter.unitId)
                          : null;
                        const subject = unit
                          ? subjects.find((s) => s.id === unit.subjectId)
                          : null;
                        const label =
                          subject && unit && chapter && topic
                            ? `${getExamName(subject.examId)} > ${capitalize(subject.name)} > ${capitalize(unit.name)} > ${capitalize(chapter.name)} > ${capitalize(topic.name)} > ${capitalize(st.name)}`
                            : capitalize(st.name);
                        const checked = quickCreateSelectedSubTopicIds.includes(
                          st.id,
                        );
                        return (
                          <label
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => toggleQuickCreateSubTopic(st.id)}
                              className="h-4 w-4"
                            />
                    ...
                    <div className="border-t px-3 py-2 text-xs text-muted-foreground">
                      {quickCreateSelectedSubTopicIds.length} subtopic
                      {quickCreateSelectedSubTopicIds.length === 1 ? "" : "s"}{" "}
                      selected
                    </div>
                  </div>
                </div>
                <div className="grid gap-3">
                  ...
                  {quickCreateSelectedSubTopicsSorted.length === 0 ? (
                    <div className="rounded-md border bg-muted/20 p-4 text-sm text-muted-foreground">
                      Select at least one subtopic to start adding definitions.
                    </div>
                  ) : (
                    <div className="grid gap-4">
                          ...
                          <div key={st.id} className="rounded-md border p-3">
                            <div className="mb-2 flex items-center justify-between">
                              ...
                              <div className="text-xs text-muted-foreground">
                                One definition per line (unique per subtopic)
                              </div>
                            </div>
                            <Textarea
                              value={
                                quickCreateDefinitionsBySubTopicId[st.id] ?? ""
                              }
                              onChange={(e) =>
                                setQuickCreateDefinitionsBySubTopicId(
                                  (prev) => ({
              ...
              <div className="mt-6 flex flex-wrap gap-2 border-t pt-4">
                ...
                <Button
                  type="button"
                  onClick={handleBulkCreateDefinitions}
                  disabled={
                    quickCreateSelectedSubTopicIds.length === 0 ||
                    quickCreateSelectedSubTopicsSorted.every(
                      (st) =>
                        !(
                          quickCreateDefinitionsBySubTopicId[st.id] || ""
                        ).trim(),
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1.5">
                    ...
                    <CardDescription className="text-sm text-muted-foreground">
                      Manage and organize your definitions, create new
                      definitions, and track definition performance across your
                    ...
                    {!isReorderingEnabled ? (
                      ...
                      <>
                        ...
                        <Button
                          size="sm"
                          className="h-9 px-3"
                          onClick={saveReorderedDefinitions}
                        >
                          Save Order
                        </Button>
                      </>
                ...
                <div className="flex flex-col gap-3 md:flex-row flex-wrap">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      {subTopicsForTopicFilter.map((st) => (
                        <SelectItem key={st.id} value={st.id}>
                          {capitalize(st.name)}
                        </SelectItem>
                      ))}
                  ...
                  <Select
                    value={metaFilter}
                    onValueChange={(v: "all" | "filled" | "not-filled") =>
                      setMetaFilter(v)
                    }
                  >
                    <SelectTrigger className="h-10 w-[180px] rounded-lg">
                  ...
                  {groupedPaged.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/60 bg-muted/20 py-16 px-6 text-center">
                      ...
                      <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                        Add definitions via Add New Definitions or Quick Create
                        for multiple subtopics at once.
                      </p>
                    </div>
                  ) : (
                    groupedPaged.map(
                      ({
                        ...
                        <Card
                          key={`${examId}-${subjectId}-${unitId}-${chapterId}-${topicId}-${subTopicId}`}
                          ...
                          <CardHeader className="pb-3 pt-4">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge
                                ...
                                className="rounded-full px-2.5 py-0.5 text-xs font-medium"
                              >
                                {groupDefs.length} definition
                                {groupDefs.length === 1 ? "" : "s"}
                              </Badge>
                            </div>
                          </CardHeader>
                              ...
                              <Table className="table-fixed min-w-[780px] w-full sm:min-w-[980px]">
                                ...
                                <TableBody>
                                  {groupDefs.map((d) => (
                                    <TableRow
                                      key={d.id}
                                      className={`transition-colors ${d.status === "Inactive" ? "opacity-60" : "hover:bg-muted/40"} ${dragOverDefinition?.id === d.id ? "border-2 border-primary/30 bg-primary/5" : ""} ${isReorderingEnabled ? "cursor-move" : "cursor-default"}`}
                                      draggable={isReorderingEnabled}
                                      onDragStart={(e) =>
                                        handleDefinitionDragStart(e, d)
                                      }
                                      onDragOver={handleDefinitionDragOver}
                                      onDragEnter={() =>
                                        handleDefinitionDragEnter(d)
                                      }
                                      onDragLeave={handleDefinitionDragLeave}
                                      onDrop={(e) => handleDefinitionDrop(e, d)}
                                    >
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                      ...
                      <span>
                        {filteredDefinitions.length === 0
                          ? 0
                          : `${startIndex + 1}-${Math.min(startIndex + pageSize, filteredDefinitions.length)} of ${filteredDefinitions.length}`}
                      </span>
                    </div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Update definition details.</DialogDescription>
          </DialogHeader>
          {editingDefinition && (
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
              ...
              <div className="grid gap-2">
                <Label htmlFor="edit-definition-order">Order Number</Label>
                <Input
                  id="edit-definition-order"
                  type="number"
                  value={editingDefinition.orderNumber}
                  onChange={(e) =>
                    setEditingDefinition((prev) =>
                      prev
                        ? {
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEditDefinition}>Save</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {definitionToDelete?.name}&quot;? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <Button variant="destructive" onClick={confirmDeleteDefinition}>
              Delete Definition
            </Button>
          </DialogFooter>
        </DialogContent>
