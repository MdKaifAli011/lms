...
export default function SubTopicsPage() {
  ...
  const [selectedChapterId, setSelectedChapterId] = React.useState<
    string | null
  >(null);
  const [addSubTopicRows, setAddSubTopicRows] = React.useState<
    { topicId: string | null; subtopicNames: string }[]
  >([{ topicId: null, subtopicNames: "" }]);

  const [editingSubTopic, setEditingSubTopic] = React.useState<SubTopic | null>(
    null,
  );
  const [subTopicToDelete, setSubTopicToDelete] =
    React.useState<SubTopic | null>(null);

  const [isReorderingEnabled, setIsReorderingEnabled] = React.useState(false);
  const [publishingId, setPublishingId] = React.useState<string | null>(null);
  const [draggedSubTopic, setDraggedSubTopic] = React.useState<SubTopic | null>(
    null,
  );
  const [dragOverSubTopic, setDragOverSubTopic] =
    React.useState<SubTopic | null>(null);

  const [quickCreateSelectedTopicIds, setQuickCreateSelectedTopicIds] =
    React.useState<string[]>([]);
  const [quickCreateSubTopicsByTopicId, setQuickCreateSubTopicsByTopicId] =
  ...
  const quickCreateSelectedTopicsSorted = React.useMemo(() => {
    ...
    quickCreateAvailableTopics,
    quickCreateSelectedTopicIds,
    chapters,
    units,
    subjects,
  ]);
  ...
  const quickCreateChaptersGrouped = React.useMemo(() => {
    const byChapter = new Map<
      string,
      { chapterId: string; label: string; topicIds: string[] }
    >();
  ...
  const handleAddSubTopics = async () => {
    ...
    if (duplicate && !created)
      toast.warning(`${duplicate} skipped (already exist in this topic)`);
  ...
  const toggleQuickCreateSelectAll = () => {
    setQuickCreateSelectedTopicIds((prev) =>
      prev.length === quickCreateAvailableTopics.length
        ? []
        : quickCreateAvailableTopics.map((t) => t.id),
    );
  };

  const toggleQuickCreateByChapter = (chapterId: string) => {
    const group = quickCreateChaptersGrouped.find(
      (g) => g.chapterId === chapterId,
    );
    if (!group) return;
    setQuickCreateSelectedTopicIds((prev) => {
  ...
  const handleBulkCreateSubTopics = async () => {
    ...
    quickCreateSelectedTopicIds.forEach((topicId) => {
      ...
      raw
        ...
        .map((s) => s.trim())
        .filter(Boolean)
        .forEach((name) => toCreate.push({ topicId, name }));
    });
    if (toCreate.length === 0) return;
    let created = 0;
    let duplicate = 0;
  ...
  const openEditDialog = (st: SubTopic) => {
  ...
  const confirmDeleteSubTopic = async () => {
    ...
    try {
      const res = await fetch(`${SUBTOPICS_API}/${subTopicToDelete.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
  ...
  const handlePublish = async (
    ...
    try {
      ...
      setSubTopics((prev) =>
        prev.map((s) =>
          s.id === updated.id ? { ...s, seo: updated.seo ?? s.seo } : s,
        ),
      );
      toast.success(
        noIndex
  ...
  const handleSubTopicDragOver = (e: React.DragEvent) => {
    e.preventDefault();
  ...
  const handleSubTopicDrop = (e: React.DragEvent, target: SubTopic) => {
    ...
    setSubTopics((prev) => {
      ...
      const group = prev
        .filter((s) => s.topicId === target.topicId)
        .slice()
        .sort((a, b) => a.orderNumber - b.orderNumber);
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredSubTopics.length / pageSize),
  );
  const safePage = Math.min(currentPage, totalPages);
  const startIndex = (safePage - 1) * pageSize;
  const pagedSubTopics = filteredSubTopics.slice(
    startIndex,
    startIndex + pageSize,
  );
  ...
  const groupedPaged = React.useMemo(() => {
    ...
    map.forEach((v) => {
      v.subTopics.sort((a, b) => a.orderNumber - b.orderNumber);
    });
  ...
  const canAddSubTopics = addSubTopicRows.some(
    (r) =>
      r.topicId &&
      r.subtopicNames
        .trim()
        ...
        <div className="flex items-center gap-2">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Sub Topics</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Select exam, subject, unit, and chapter. Then select topic(s)
                  and enter subtopic names for each topic (one per line).
                </DialogDescription>
              </DialogHeader>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={addAnotherTopicRow}
                    >
                      Add Another Topic
                    </Button>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Select topics and enter subtopics for each topic separately.
                    Each topic has its own textarea.
                  </p>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <Select
                            value={row.topicId ?? ""}
                            onValueChange={(v) =>
                              updateAddRow(index, "topicId", v || null)
                            }
                        ...
                        <Textarea
                          ...
                          value={row.subtopicNames}
                          onChange={(e) =>
                            updateAddRow(index, "subtopicNames", e.target.value)
                          }
        ...
        {isQuickCreateOpen ? (
          <Card className="border-border/80 shadow-sm">
            <CardHeader className="space-y-1">
              ...
              <CardDescription>
                Select one or more topics and enter subtopic names for each
                topic (one per line). Content and meta can be added later.
              </CardDescription>
            </CardHeader>
            <Separator />
            <CardContent className="pt-6">
                ...
                <div className="grid gap-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      >
                        {quickCreateSelectedTopicIds.length ===
                        quickCreateAvailableTopics.length
                          ? "Clear all"
                          : "Select all"}
                      </Button>
                      {quickCreateChaptersGrouped.length > 1 && (
                        <>
                          ...
                          {quickCreateChaptersGrouped.map(
                            ({ chapterId, label, topicIds }) => {
                              const allIn = topicIds.every((id) =>
                                quickCreateSelectedTopicIds.includes(id),
                              );
                              return (
                                <Button
                                  key={chapterId}
                                  ...
                                  onClick={() =>
                                    toggleQuickCreateByChapter(chapterId)
                                  }
                                >
                                  {allIn ? "✓ " : ""}
                                  {label}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Click a chapter to select or clear all its topics at once.
                  </p>
                  <div className="rounded-md border bg-background">
                      ...
                      {quickCreateAvailableTopics.map((topic) => {
                        ...
                        const label =
                          subject && unit && chapter
                            ? `${getExamName(subject.examId)} > ${subject.name} > ${unit.name} > ${chapter.name} > ${topic.name}`
                            : topic.name;
                        const checked = quickCreateSelectedTopicIds.includes(
                          topic.id,
                        );
                        return (
                          <label
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => toggleQuickCreateTopic(topic.id)}
                              className="h-4 w-4"
                            />
                    ...
                    <div className="border-t px-3 py-2 text-xs text-muted-foreground">
                      {quickCreateSelectedTopicIds.length} topic
                      {quickCreateSelectedTopicIds.length === 1 ? "" : "s"}{" "}
                      selected
                    </div>
                  </div>
                </div>
                <div className="grid gap-3">
                  ...
                  {quickCreateSelectedTopicsSorted.length === 0 ? (
                    <div className="rounded-md border bg-muted/20 p-4 text-sm text-muted-foreground">
                      Select at least one topic to start adding subtopics.
                    </div>
                  ) : (
                    <div className="grid gap-4">
                      {quickCreateSelectedTopicsSorted.map((topic) => {
                        ...
                        return (
                          <div key={topic.id} className="rounded-md border p-3">
                            ...
                            <Textarea
                              value={
                                quickCreateSubTopicsByTopicId[topic.id] ?? ""
                              }
                              onChange={(e) =>
                                setQuickCreateSubTopicsByTopicId((prev) => ({
                                  ...prev,
              ...
              <div className="mt-6 flex flex-wrap gap-2 border-t pt-4">
                ...
                <Button
                  type="button"
                  onClick={handleBulkCreateSubTopics}
                  disabled={
                    quickCreateSelectedTopicIds.length === 0 ||
                    quickCreateSelectedTopicsSorted.every(
                      (t) =>
                        !(quickCreateSubTopicsByTopicId[t.id] || "").trim(),
                    )
                  }
                >
                  Create Sub Topics
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1.5">
                    ...
                    <CardDescription className="text-sm text-muted-foreground">
                      Manage and organize your subtopics, create new subtopics,
                      and track subtopic performance across your educational
                      platform.
                    </CardDescription>
                  </div>
                    ...
                    {!isReorderingEnabled ? (
                      ...
                      <>
                        ...
                        <Button
                          size="sm"
                          className="h-9 px-3"
                          onClick={saveReorderedSubTopics}
                        >
                          Save Order
                        </Button>
                      </>
                  ...
                  {groupedPaged.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/60 bg-muted/20 py-16 px-6 text-center">
                      ...
                      <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                        Add subtopics via Add New Sub Topics or Quick Create for
                    ...
                    groupedPaged.map(
                      ({
                        ...
                        <Card
                          key={`${examId}-${subjectId}-${unitId}-${chapterId}-${topicId}`}
                          ...
                          <CardHeader className="pb-3 pt-4">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <Badge
                                ...
                                className="rounded-full px-2.5 py-0.5 text-xs font-medium"
                              >
                                {groupSubTopics.length} subtopic
                                {groupSubTopics.length === 1 ? "" : "s"}
                              </Badge>
                            </div>
                          </CardHeader>
                              ...
                              <Table className="table-fixed min-w-[780px] w-full sm:min-w-[980px]">
                                ...
                                <TableBody>
                                  {groupSubTopics.map((st) => (
                                    <TableRow
                                      key={st.id}
                                      className={`transition-colors ${st.status === "Inactive" ? "opacity-60" : "hover:bg-muted/40"} ${dragOverSubTopic?.id === st.id ? "border-2 border-primary/30 bg-primary/5" : ""} ${isReorderingEnabled ? "cursor-move" : "cursor-default"}`}
                                      draggable={isReorderingEnabled}
                                      onDragStart={(e) =>
                                        handleSubTopicDragStart(e, st)
                                      }
                                      onDragOver={handleSubTopicDragOver}
                                      onDragEnter={() =>
                                        handleSubTopicDragEnter(st)
                                      }
                                      onDragLeave={handleSubTopicDragLeave}
                                      onDrop={(e) => handleSubTopicDrop(e, st)}
                                    >
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                      ...
                      <span>
                        {filteredSubTopics.length === 0
                          ? 0
                          : `${startIndex + 1}-${Math.min(startIndex + pageSize, filteredSubTopics.length)} of ${filteredSubTopics.length}`}
                      </span>
                    </div>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Update subtopic details.</DialogDescription>
          </DialogHeader>
          {editingSubTopic && (
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label htmlFor="edit-subtopic-name">Sub Topic Name *</Label>
                <Input
              ...
              <div className="grid gap-2">
                <Label htmlFor="edit-subtopic-order">Order Number</Label>
                <Input
                  id="edit-subtopic-order"
                  type="number"
                  value={editingSubTopic.orderNumber}
                  onChange={(e) =>
                    setEditingSubTopic((prev) =>
                      prev
                        ? {
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {subTopicToDelete?.name}&quot;? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
