...
export default function SubjectSlugPage({
  params,
}: {
  ...
  React.useEffect(() => {
    params.then((p) => setSubjectId(p.slug))
  }, [params])
  ...
  React.useEffect(() => {
    ...
    const html = typeof subject.contentBody === "string" ? subject.contentBody : ""
  ...
  }, [subject?.id, subject?.contentBody])

  React.useEffect(() => {
    if (!subjectId) return
    let cancelled = false
    setLoaded(false)
    fetch(`${API_BASE}/${encodeURIComponent(subjectId)}`)
      .then((res) => {
        if (cancelled) return
        if (!res.ok) {
          setSubject(null)
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbLink href="/self-study/subjects">Subjects</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/subjects">Subjects</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Subject</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </header>
        <div className="flex flex-1 flex-col gap-4 p-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>No subject matches this link. It may have been removed or the URL is incorrect.</CardDescription>
            </CardHeader>
            <CardContent>
              <Button asChild variant="outline">
                <Link href="/self-study/subjects">Back to Subjects</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
  ...
  return (
    <div className="flex min-h-0 flex-1 min-w-0 flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/subjects">Subjects</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>{capitalize(subject.name)}</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
        <div className="flex items-center gap-3">
      ...
      <ContentSeoLayout
        ...
        seoSidebar={<SeoSettingsForm seo={seo} setSeo={setSeo} idPrefix="subject-seo" />}
      />
    </div>
  )
}
