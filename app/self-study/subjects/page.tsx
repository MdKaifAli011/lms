...
export default function SubjectsPage() {
  ...
  const [metaStatusFilter, setMetaStatusFilter] = React.useState<
  ...
  const [isSingleAddDialogOpen, setIsSingleAddDialogOpen] =
    React.useState(false);
  ...
  const [draggedSubject, setDraggedSubject] = React.useState<Subject | null>(
    null,
  );
  const [dragOverSubject, setDragOverSubject] = React.useState<Subject | null>(
    null,
  );

  const [quickCreateSelectedExamIds, setQuickCreateSelectedExamIds] =
    React.useState<string[]>([]);
  const [quickCreateSubjectsByExamId, setQuickCreateSubjectsByExamId] =
    React.useState<Record<string, string>>({});

  const [singleNewSubject, setSingleNewSubject] = React.useState<{
    name: string;
    examId: string | null;
  }>({
    name: "",
  ...
  const handleBulkCreateSubjects = async () => {
    ...
    if (duplicate)
      toast.warning(`${duplicate} skipped (already exist in this exam)`);
  ...
  const confirmDeleteSubject = async () => {
    ...
    try {
      const res = await fetch(`${SUBJECTS_API}/${subjectToDelete.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setSingleNewSubject((p) => ({ ...p, examId: v || null }))
                    }
                  >
                    <SelectTrigger>
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    value={String(
                      singleNewSubject.examId
                        ? getNextOrderNumber(singleNewSubject.examId)
                        : "-",
                    )}
                    readOnly
                    className="bg-muted"
        ...
        {isQuickCreateSheetOpen ? (
          <Card className="border-border/80 shadow-sm">
            <CardHeader className="space-y-1">
              ...
              <CardDescription>
                Select one or more exams and enter subject names for each exam
                separately (one per line). Content and SEO can be added later.
              </CardDescription>
            </CardHeader>
            <Separator />
              ...
              <div className="grid gap-4">
                <div className="grid gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {quickCreateSelectedExamIds.length === exams.length
                        ? "Clear All"
                        : "Select All"}
                    </Button>
                  </div>
                  <div className="rounded-md border bg-background">
                    <div className="max-h-56 overflow-auto p-2">
                      {exams.map((exam) => {
                        const checked = quickCreateSelectedExamIds.includes(
                          exam.id,
                        );
                        return (
                          <label
            ...
            <Card className="shadow-sm">
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1">
                    ...
                    <CardDescription className="text-sm">
                      Manage subjects, organize content, and configure learning
                      paths
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {exams
                    .filter((exam) =>
                      pagedSubjects.some((s) => s.examId === exam.id),
                    )
                    .map((exam) => {
                            ...
                            <Table className="table-fixed">
                              <TableHeader>
                                <TableRow className="border-b-2">
                                  <TableHead className="font-semibold text-xs uppercase tracking-wider w-[88px]">
                                    Order
                                  </TableHead>
                                  <TableHead className="font-semibold text-xs uppercase tracking-wider w-[320px]">
                                    Subject Name
                                  </TableHead>
                                  <TableHead className="font-semibold text-xs uppercase tracking-wider w-[180px]">
                                    Content
                                  </TableHead>
                                  <TableHead className="font-semibold text-xs uppercase tracking-wider w-[80px]">
                                    Meta
                                  </TableHead>
                                  ...
                                  <TableHead className="text-right font-semibold text-xs uppercase tracking-wider w-[120px] pr-2">
                                    Actions
                                  </TableHead>
                                </TableRow>
                              </TableHeader>
                ...
                <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>
                      {filteredSubjects.length === 0 ? 0 : startIndex + 1}-
                      {Math.min(startIndex + pageSize, filteredSubjects.length)}{" "}
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setEditingSubject({ ...editingSubject, examId: v })
                  }
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select an exam" />
