...
export default function UnitSlugPage({
  params,
}: {
  ...
  React.useEffect(() => {
    params.then((p) => setUnitId(p.slug))
  }, [params])
  ...
  React.useEffect(() => {
    if (!unit?.id) return
    const html = typeof unit.contentBody === "string" ? unit.contentBody : ""
  ...
  }, [unit?.id, unit?.contentBody])

  React.useEffect(() => {
    if (!unitId) return
    let cancelled = false
    setLoaded(false)
    fetch(`${API_BASE}/${encodeURIComponent(unitId)}`)
      .then((res) => {
        if (cancelled) return
        if (!res.ok) {
          setUnit(null)
  ...
  const saveContentAndSeo = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/${unit.id}`, {
        method: "PUT",
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbLink href="/self-study/units">Units</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/units">Units</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Unit</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </header>
        <div className="flex flex-1 flex-col gap-4 p-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>No unit matches this link. It may have been removed or the URL is incorrect.</CardDescription>
            </CardHeader>
            <CardContent>
              <Button asChild variant="outline">
                <Link href="/self-study/units">Back to Units</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/units">Units</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>{capitalize(unit.name)}</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
        <div className="flex items-center gap-3">
