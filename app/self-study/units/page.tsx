...
export default function UnitsPage() {
  ...
  const [isQuickCreateSheetOpen, setIsQuickCreateSheetOpen] =
  ...
  const [selectedSubjectId, setSelectedSubjectId] = React.useState<
    string | null
  >(null);
  const [addUnitRows, setAddUnitRows] = React.useState<
  ...
  const [quickCreateSelectedSubjectIds, setQuickCreateSelectedSubjectIds] =
    React.useState<string[]>([]);
  const [quickCreateUnitsBySubjectId, setQuickCreateUnitsBySubjectId] =
  ...
  const quickCreateAvailableSubjects = React.useMemo(
    () =>
      subjects.filter(
        (s) =>
          availableExams.some((e) => e.id === s.examId) &&
          s.status !== "Inactive",
      ),
  ...
  const quickCreateSelectedSubjectsSorted = React.useMemo(() => {
    ...
    quickCreateAvailableSubjects,
    quickCreateSelectedSubjectIds,
    exams,
    subjects,
  ]);
  ...
  const quickCreateExamsGrouped = React.useMemo(() => {
    const byExam = new Map<
      string,
      { examId: string; examName: string; subjectIds: string[] }
    >();
  ...
  const toggleQuickCreateByExam = (examId: string) => {
    ...
    setQuickCreateSelectedSubjectIds((prev) => {
      ...
      if (allSelected)
        return prev.filter((id) => !group.subjectIds.includes(id));
  ...
  const handleBulkCreateUnits = async () => {
    ...
    if (duplicate)
      toast.warning(`${duplicate} skipped (already exist in this subject)`);
    ...
    try {
      const res = await fetch(`${UNITS_API}/${editingUnit.id}`, {
        ...
        body: JSON.stringify({
          ...
          orderNumber: editingUnit.orderNumber,
        }),
      });
      if (res.status === 409) {
  ...
  const confirmDeleteUnit = async () => {
    ...
    try {
      const res = await fetch(`${UNITS_API}/${unitToDelete.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
  ...
  const handlePublish = async (
    ...
    try {
      ...
      setUnits((prev) =>
        prev.map((u) =>
          u.id === updated.id ? { ...u, seo: updated.seo ?? u.seo } : u,
        ),
      );
      toast.success(
        noIndex
  ...
  const filteredUnits = React.useMemo(() => {
    return units.filter((u) => {
      ...
      const matchesSubject =
        subjectFilter === "all" || String(u.subjectId) === subjectFilter;
      const metaFilled = !!(
  ...
  const groupedPaged = React.useMemo(() => {
    const map = new Map<
      string,
      { examId: string; subjectId: string; units: Unit[] }
    >();
            ...
            <DialogContent className="sm:max-w-[500px]">
                ...
                <div className="space-y-2">
                  ...
                  {selectedSubjectId && (
                    <p className="text-xs text-muted-foreground">
                      Order Number: next will start at{" "}
                      {getNextOrderForSubject(selectedSubjectId)}
                    </p>
                  )}
                </div>
              </div>
              <DialogFooter>
                ...
                <Button
                  ...
                  disabled={
                    !selectedSubjectId ||
                    addUnitRows.every((r) => !r.name.trim())
                  }
                >
                  Add Units
                </Button>
        ...
        {isQuickCreateSheetOpen ? (
          <Card className="border-border/80 shadow-sm">
            <CardHeader className="space-y-1">
              ...
              <CardDescription>
                Select one or more subjects and enter unit names for each
                subject separately (one per line). Content and meta can be added
                later.
              </CardDescription>
            </CardHeader>
            <Separator />
                ...
                <div className="grid gap-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      >
                        {quickCreateSelectedSubjectIds.length ===
                        quickCreateAvailableSubjects.length
                          ? "Clear all"
                          : "Select all"}
                      </Button>
                      {quickCreateExamsGrouped.length > 1 && (
                        <>
                          ...
                          {quickCreateExamsGrouped.map(
                            ({ examId, examName, subjectIds }) => {
                              ...
                              return (
                                <Button
                                  ...
                                  onClick={() =>
                                    toggleQuickCreateByExam(examId)
                                  }
                                >
                                  {allIn ? "✓ " : ""}
                                  {examName}
                  ...
                  <p className="text-xs text-muted-foreground">
                    Click an exam name to select or clear all its subjects at
                    once.
                  </p>
                  <div className="rounded-md border bg-background">
                    <div className="max-h-56 overflow-auto p-2">
                      {quickCreateAvailableSubjects.map((subject) => {
                        const checked = quickCreateSelectedSubjectIds.includes(
                          subject.id,
                        );
                        return (
                          <label
                          ...
                          >
                            <input
                              ...
                              checked={checked}
                              onChange={() =>
                                toggleQuickCreateSubject(subject.id)
                              }
                              className="h-4 w-4"
                            />
                            <span className="text-sm">
                              {getExamName(subject.examId)} &gt;{" "}
                    ...
                    <div className="border-t px-3 py-2 text-xs text-muted-foreground">
                      ...
                      {quickCreateSelectedSubjectIds.length === 1
                        ? ""
                        : "s"}{" "}
                      selected
                    </div>
                      ...
                      {quickCreateSelectedSubjectsSorted.map((subject) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Textarea
                              value={
                                quickCreateUnitsBySubjectId[subjectId] ?? ""
                              }
                              onChange={(e) =>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1.5">
                    ...
                    <CardDescription className="text-sm text-muted-foreground">
                      Manage and organize your units, create new units, and
                      track unit performance across your educational platform.
                    </CardDescription>
                  </div>
                  ...
                  {groupedPaged.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/60 bg-muted/20 py-16 px-6 text-center">
                      ...
                      <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                        Add units by choosing an exam and subject above, or use
                        Quick Create to add units for multiple subjects at once.
                      </p>
                    </div>
                  ) : (
                    groupedPaged.map(
                      ({ examId, subjectId, units: groupUnits }) => (
                        <Card
                            ...
                            <div className="w-full overflow-x-auto">
                              <Table className="table-fixed min-w-[780px] w-full sm:min-w-[980px]">
                                ...
                                `}
                                      ...
                                      onDragStart={(e) =>
                                        handleUnitDragStart(e, unit)
                                      }
                                      onDragOver={handleUnitDragOver}
                                      onDragEnter={() =>
                                        handleUnitDragEnter(unit)
                                      }
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {unitToDelete ? capitalize(unitToDelete.name) : ""}&quot;? This
