...
export default function ChaptersPage() {
  ...
  const [addChapterRows, setAddChapterRows] = React.useState<
  ...
  const [chapterToDelete, setChapterToDelete] = React.useState<Chapter | null>(
    null,
  );

  const [isReorderingEnabled, setIsReorderingEnabled] = React.useState(false);
  const [publishingId, setPublishingId] = React.useState<string | null>(null);
  const [draggedChapter, setDraggedChapter] = React.useState<Chapter | null>(
    null,
  );
  const [dragOverChapter, setDragOverChapter] = React.useState<Chapter | null>(
    null,
  );

  const [quickCreateSelectedUnitIds, setQuickCreateSelectedUnitIds] =
    React.useState<string[]>([]);
  const [quickCreateChaptersByUnitId, setQuickCreateChaptersByUnitId] =
  ...
  const quickCreateSubjectsGrouped = React.useMemo(() => {
    const bySubject = new Map<
      string,
      { subjectId: string; label: string; unitIds: string[] }
    >();
  ...
  const toggleQuickCreateBySubject = (subjectId: string) => {
    const group = quickCreateSubjectsGrouped.find(
  ...
  const handleBulkCreateChapters = async () => {
    ...
    if (duplicate)
      toast.warning(`${duplicate} skipped (already exist in this unit)`);
  ...
  const confirmDeleteChapter = async () => {
    ...
    try {
      const res = await fetch(`${CHAPTERS_API}/${chapterToDelete.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
  ...
  const handleToggleStatus = async (chapterId: string) => {
    ...
    try {
      ...
      setChapters((prev) =>
        prev.map((c) =>
          c.id === chapterId ? { ...c, status: nextStatus } : c,
        ),
      );
    } catch {
  ...
  const handlePublish = async (
    ...
    try {
      ...
      setChapters((prev) =>
        prev.map((c) =>
          c.id === updated.id ? { ...c, seo: updated.seo ?? c.seo } : c,
        ),
      );
      toast.success(
        noIndex
  ...
  const filteredChapters = React.useMemo(() => {
    return chapters.filter((c) => {
      ...
      const matchesUnit =
        unitFilter === "all" || String(c.unitId) === unitFilter;
      const metaFilled = !!(
  ...
  const groupedPaged = React.useMemo(() => {
    const map = new Map<
      string,
      { examId: string; subjectId: string; unitId: string; chapters: Chapter[] }
    >();
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-2">
                    ...
                    {addChapterRows.map((row, index) => (
                      <div key={index} className="flex gap-2">
                        <Input
                          ...
                          value={row.name}
                          onChange={(e) =>
                            updateAddRow(index, "name", e.target.value)
                          }
                          className="flex-1"
                        />
                        <Input
                  ...
                  {selectedUnitId && (
                    <p className="text-xs text-muted-foreground">
                      Order: next will start at{" "}
                      {getNextOrderForUnit(selectedUnitId)}
                    </p>
                  )}
                </div>
              </div>
              <DialogFooter>
                ...
                <Button
                  ...
                  disabled={
                    !selectedUnitId ||
                    addChapterRows.every((r) => !r.name.trim())
                  }
                >
                  Add Chapters
                </Button>
        ...
        {isQuickCreateOpen ? (
          <Card className="border-border/80 shadow-sm">
            <CardHeader className="space-y-1">
              ...
              <CardDescription>
                Select one or more units and enter chapter names for each unit
                (one per line). Content and meta can be added later.
              </CardDescription>
            </CardHeader>
            <Separator />
                ...
                <div className="grid gap-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      ...
                      >
                        {quickCreateSelectedUnitIds.length ===
                        quickCreateAvailableUnits.length
                          ? "Clear all"
                          : "Select all"}
                      </Button>
                      {quickCreateSubjectsGrouped.length > 1 && (
                        <>
                          ...
                          {quickCreateSubjectsGrouped.map(
                            ({ subjectId, label, unitIds }) => {
                              ...
                              return (
                                <Button
                                  ...
                                  onClick={() =>
                                    toggleQuickCreateBySubject(subjectId)
                                  }
                                >
                                  {allIn ? "✓ " : ""}
                                  {label}
                  ...
                  <div className="rounded-md border bg-background">
                    <div className="max-h-56 overflow-auto p-2">
                      {quickCreateAvailableUnits.map((unit) => {
                        const subject = subjects.find(
                          (s) => s.id === unit.subjectId,
                        );
                        const label = subject
                          ? `${getExamName(subject.examId)} > ${subject.name} > ${unit.name}`
                          : unit.name;
                        const checked = quickCreateSelectedUnitIds.includes(
                          unit.id,
                        );
                        return (
                          <label
                    ...
                    <div className="border-t px-3 py-2 text-xs text-muted-foreground">
                      ...
                      {quickCreateSelectedUnitIds.length === 1 ? "" : "s"}{" "}
                      selected
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1.5">
                    ...
                    <CardDescription className="text-sm text-muted-foreground">
                      Manage and organize your chapters, create new chapters,
                      and track chapter performance across your educational
                      platform.
                    </CardDescription>
                  </div>
                  ...
                  {groupedPaged.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/60 bg-muted/20 py-16 px-6 text-center">
                      ...
                      <p className="mt-1 max-w-sm text-sm text-muted-foreground">
                        Add chapters via Add New Chapter or Quick Create for
                                ...
                                <TableBody>
                                  {groupChapters.map((chapter) => (
                                    ...
                                    `}
                                      ...
                                      onDragStart={(e) =>
                                        handleChapterDragStart(e, chapter)
                                      }
                                      onDragOver={handleChapterDragOver}
                                      onDragEnter={() =>
                                        handleChapterDragEnter(chapter)
                                      }
                                      onDragLeave={handleChapterDragLeave}
                                      onDrop={(e) =>
                                        handleChapterDrop(e, chapter)
                                      }
                                    >
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {chapterToDelete ? capitalize(chapterToDelete.name) : ""}&quot;?
