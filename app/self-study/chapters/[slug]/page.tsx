...
export default function ChapterSlugPage({
  ...
  React.useEffect(() => {
    params.then((p) => setChapterId(p.slug))
  }, [params])
  ...
  React.useEffect(() => {
    ...
    const html = typeof chapter.contentBody === "string" ? chapter.contentBody : ""
  ...
  }, [chapter?.id, chapter?.contentBody])

  React.useEffect(() => {
    if (!chapterId) return
    let cancelled = false
    setLoaded(false)
    fetch(`${API_BASE}/${encodeURIComponent(chapterId)}`)
      .then((res) => {
        if (cancelled) return
        if (!res.ok) {
          setChapter(null)
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbLink href="/self-study/chapters">Chapters</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/chapters">Chapters</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Chapter</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </header>
        <div className="flex flex-1 flex-col gap-4 p-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>No chapter matches this link. It may have been removed or the URL is incorrect.</CardDescription>
            </CardHeader>
            <CardContent>
              <Button asChild variant="outline">
                <Link href="/self-study/chapters">Back to Chapters</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/chapters">Chapters</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>{capitalize(chapter.name)}</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
        <div className="flex items-center gap-3">
