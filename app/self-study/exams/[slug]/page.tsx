...
export default function ExamSlugPage({
  ...
  React.useEffect(() => {
    if (!exam?.id) return
    const html = typeof exam.contentBody === "string" ? exam.contentBody : ""
  ...
  const saveContentAndSeo = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/${exam.id}`, {
        method: "PUT",
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbLink href="/self-study/exams">Exams</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
        ...
        <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border/60 bg-background/95 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/exams">Exams</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage>Exam</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </header>
        <div className="flex flex-1 flex-col gap-4 p-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>No exam matches this link. It may have been removed or the URL is incorrect.</CardDescription>
            </CardHeader>
            <CardContent>
              <Button asChild variant="outline">
                <Link href="/self-study/exams">Back to Exams</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
  ...
  return (
    <div className="flex min-h-0 flex-1 min-w-0 flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem><BreadcrumbLink href="/self-study/exams">Exams</BreadcrumbLink></BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem><BreadcrumbPage className="capitalize">{exam.name}</BreadcrumbPage></BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
        <div className="flex items-center gap-3">
      ...
      <ContentSeoLayout
        ...
        seoSidebar={<SeoSettingsForm seo={seo} setSeo={setSeo} idPrefix="exam-seo" />}
      />
    </div>
  )
}
