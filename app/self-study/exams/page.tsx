...
export default function ExamsPage() {
  ...
  const createUniqueSlug = (name: string, options?: { excludeId?: string }) => {
    ...
    const taken = new Set(
      exams
        .filter((e) => e.id !== options?.excludeId)
        .map((e) => e.slug)
    )
  ...
  const handleDrop = (e: React.DragEvent, targetExam: Exam) => {
    ...
    if (draggedExam && draggedExam.id !== targetExam.id) {
      ...
      const draggedIndex = newExams.findIndex(exam => exam.id === draggedExam.id)
      const targetIndex = newExams.findIndex(exam => exam.id === targetExam.id)
  ...
  const handleAddExam = async () => {
    ...
    try {
      ...
      setNewExam({ name: "", status: "Active", cardImageUrl: "", descriptions: ["", "", "", ""] })
  ...
  const handleEditExam = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        const msg = (typeof data === "object" && data?.error) ? data.error : (res.statusText || "Failed to update")
        throw new Error(msg)
      }
  ...
  const confirmDeleteExam = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/${examToDelete.id}`, { method: "DELETE" })
  ...
  const openEditDialog = (exam: Exam) => {
    setEditingExam({
      ...exam,
      cardImageUrl: (exam.image && exam.image !== "No Image" ? exam.image : "") || "",
  ...
  return (
    <div className="flex min-h-0 flex-1 min-w-0 flex-col">
      ...
      {loading && (
        <div className="flex flex-1 items-center justify-center p-8 text-muted-foreground">Loading exams…</div>
      )}
      {!loading && (
      <>
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="card-image-url">Card Image URL (Optional)</Label>
                    <Input
                      id="card-image-url"
                      placeholder="Enter image URL (e.g., https://example.com/image.png)"
                      value={newExam.cardImageUrl}
                      onChange={(e) => setNewExam({ ...newExam, cardImageUrl: e.target.value })}
                    />
                  </div>
                    ...
                    {newExam.descriptions.map((desc, index) => (
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setNewExam({ ...newExam, descriptions: newDescriptions })
                        }}
                      />
                    ))}
                  </div>
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleAddExam} disabled={addSaving}>{addSaving ? "Saving…" : "Add Exam"}</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </header>
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
            <Card className="relative overflow-hidden">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 pt-4">
                <CardTitle className="text-[13px] font-medium">Total Exams</CardTitle>
                <div className="h-7 w-7 rounded-full bg-blue-100 flex items-center justify-center">
                  <div className="h-3 w-3 rounded-full bg-blue-500"></div>
                </div>
              </CardHeader>
              <CardContent className="pt-0 pb-4">
                <div className="text-xl font-bold leading-none">{exams.length}</div>
                <p className="text-[11px] text-muted-foreground mt-0.5">All exams in system</p>
              </CardContent>
            </Card>
            <Card className="relative overflow-hidden">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 pt-4">
                <CardTitle className="text-[13px] font-medium">Active Exams</CardTitle>
            ...
            <CardContent className="min-w-0 overflow-hidden">
                ...
                <Table className="table-fixed min-w-[780px] w-full sm:min-w-[980px]">
                  <TableHeader>
                    <TableRow className="border-b-2">
                      <TableHead className="w-16 shrink-0 font-semibold text-xs uppercase tracking-wider sm:w-[88px]">Order</TableHead>
                      <TableHead className="min-w-[140px] font-semibold text-xs uppercase tracking-wider sm:min-w-[200px]">Exam Details</TableHead>
                      <TableHead className="w-20 shrink-0 font-semibold text-xs uppercase tracking-wider sm:w-[110px]">Preview</TableHead>
                      <TableHead className="min-w-[100px] font-semibold text-xs uppercase tracking-wider sm:min-w-[140px]">Content</TableHead>
                      <TableHead className="w-20 shrink-0 font-semibold text-xs uppercase tracking-wider sm:w-[120px]">Meta</TableHead>
                      <TableHead className="w-24 shrink-0 font-semibold text-xs uppercase tracking-wider sm:w-[120px]">Visits</TableHead>
                      <TableHead className="w-14 shrink-0 font-semibold text-xs uppercase tracking-wider sm:w-[80px]">Today</TableHead>
                      <TableHead className="w-[140px] shrink-0 text-right font-semibold text-xs uppercase tracking-wider pr-2 sm:w-[190px]">Actions</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {pagedExams.map((exam) => (
                        ...
                        <div className="flex items-center gap-3">
                          <Link
                            ...
                            className={`font-semibold text-base hover:underline focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 rounded ${
                              exam.status === "Inactive" ? "line-through text-muted-foreground" : "text-foreground"
                            }`}
                          >
                      ...
                      <TableCell className={`text-muted-foreground ${
                        ...
                        <div className="w-12 h-12 bg-gradient-to-br from-muted to-muted/80 rounded-lg flex items-center justify-center text-[10px] font-medium shadow-sm overflow-hidden">
                          {/^https?:\/\//i.test(exam.image) ? (
                            ...
                            <img src={exam.image} alt="" className="h-full w-full object-cover" />
                          ) : (
                            "No Image"
                          )}
                        </div>
                      ...
                      <TableCell className={`text-muted-foreground ${
                        ...
                        {exam.content !== "-" ? exam.content : "unavailable"}
                      </TableCell>
                      <TableCell className={`text-muted-foreground ${
                        exam.status === "Inactive" ? "line-through" : ""
                      }`}>
                      ...
                      <TableCell className={exam.status === "Inactive" ? "opacity-60" : ""}>
                        <div>
                      ...
                      <TableCell className={exam.status === "Inactive" ? "line-through opacity-60" : ""}>{exam.today}</TableCell>
                      <TableCell className="text-right py-4 pr-2">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button 
                            ...
                            asChild
                          >
                            <Link href={`/self-study/exams/${encodeURIComponent(exam.slug)}`}>
              ...
              <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>
                    {filteredExams.length === 0 ? 0 : startIndex + 1}-{Math.min(startIndex + pageSize, filteredExams.length)} of {filteredExams.length}
                  </span>
                </div>
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            {editingExam && (
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="edit-card-image">Card Image URL (Optional)</Label>
                  <Input
                    id="edit-card-image"
                    value={editingExam.cardImageUrl || ""}
                    onChange={(e) =>
                      setEditingExam({ ...editingExam, cardImageUrl: e.target.value })
                    }
                  />
                </div>
                <div className="grid gap-2">
                  <Label>Card Descriptions (Max 4 items - Optional)</Label>
                  {(editingExam.descriptions || ["", "", "", ""]).map((desc, index) => (
                    <Input
                      key={index}
                      placeholder={`Description item ${index + 1}`}
                      value={desc}
                      onChange={(e) => {
                        const newDescriptions = [...(editingExam.descriptions || ["", "", "", ""])]
                        newDescriptions[index] = e.target.value
                        setEditingExam({ ...editingExam, descriptions: newDescriptions })
                      }}
                    />
                  ))}
                </div>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleEditExam} disabled={editSaving}>{editSaving ? "Saving…" : "Update Exam"}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Are you sure you want to delete the exam &quot;<span className="capitalize">{examToDelete?.name}</span>&quot;? This action cannot be undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              ...
              <Button variant="destructive" onClick={confirmDeleteExam} disabled={deleteSaving}>
