...
const API_BASE = "/api/previous-year-paper";
...
export default function PreviousYearsPage() {
  ...
  const [paperToDelete, setPaperToDelete] = React.useState<PreviousYearPaper | null>(null);
  const [editingPaper, setEditingPaper] = React.useState<PreviousYearPaper | null>(null);
  const [draggedPaper, setDraggedPaper] = React.useState<PreviousYearPaper | null>(null);
  const [dragOverPaper, setDragOverPaper] = React.useState<PreviousYearPaper | null>(null);
  ...
  const fetchPapers = React.useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to load previous year papers");
      setPapers([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Failed to create previous year paper";
      setError(msg);
      toast.error(msg);
    } finally {
      setAddSaving(false);
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Failed to update previous year paper";
      setError(msg);
      toast.error(msg);
    } finally {
      setEditSaving(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Failed to delete previous year paper";
      setError(msg);
      toast.error(msg);
    } finally {
  ...
  const handleToggleStatus = async (paper: PreviousYearPaper) => {
  ...
  const handleToggleLock = async (paper: PreviousYearPaper) => {
  ...
  const openEditDialog = (paper: PreviousYearPaper) => {
  ...
  const handleDragEnter = (paper: PreviousYearPaper) => {
  ...
  const filteredPapers = papers.filter((paper) => {
    ...
    return matchesSearch && matchesStatus && matchesExam && matchesYear;
  });

  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, examFilter, yearFilter, pageSize]);
  ...
  const renderForm = (isEdit = false) => (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Input
          id="title"
          placeholder="e.g. JEE Main 2023 Question Paper"
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="session"
            placeholder="e.g. January, April, Morning"
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Create a new previous year exam paper for student practice.
                  </DialogDescription>
                </DialogHeader>
                {renderForm()}
                <DialogFooter>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1">
                    ...
                    <CardDescription className="text-sm">
                      Manage previous year exam papers for student practice
                    </CardDescription>
                  </div>
                  <div className="flex items-center space-x-2">
                    {!isReorderingEnabled ? (
                      ...
                      {pagedPapers.map((paper) => (
                        ...
                        >
                          ...
                          >
                            ...
                            {paper.session && (
                              <div className="text-xs text-muted-foreground">{paper.session}</div>
                            )}
                          </TableCell>
                          <TableCell
                            className={paper.status === "Inactive" ? "opacity-60" : ""}
            ...
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
              ...
              <DialogFooter>
                ...
                <Button onClick={handleEdit} disabled={editSaving}>
                  {editSaving ? "Saving…" : "Update Paper"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
