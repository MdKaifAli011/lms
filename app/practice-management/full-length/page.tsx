...
const API_BASE = "/api/full-length-mock";
...
export default function FullLengthPage() {
  const [papers, setPapers] = React.useState<FullLengthMock[]>([]);
  ...
  const [paperToDelete, setPaperToDelete] = React.useState<FullLengthMock | null>(null);
  const [editingPaper, setEditingPaper] = React.useState<FullLengthMock | null>(null);
  const [draggedPaper, setDraggedPaper] = React.useState<FullLengthMock | null>(null);
  const [dragOverPaper, setDragOverPaper] = React.useState<FullLengthMock | null>(null);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.success("Full-length mock test created successfully");
      setIsAddDialogOpen(false);
      resetForm();
      fetchPapers();
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Failed to create mock test";
      setError(msg);
      toast.error(msg);
    } finally {
      setAddSaving(false);
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      toast.success("Full-length mock test updated successfully");
      setIsEditDialogOpen(false);
      setEditingPaper(null);
      resetForm();
      fetchPapers();
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Failed to update mock test";
      setError(msg);
      toast.error(msg);
    } finally {
      setEditSaving(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Failed to delete mock test";
      setError(msg);
      toast.error(msg);
    } finally {
  ...
  const handleToggleStatus = async (paper: FullLengthMock) => {
  ...
  const handleToggleLock = async (paper: FullLengthMock) => {
    ...
    try {
      ...
      toast.success(nextLocked ? "Mock test locked" : "Mock test unlocked");
    } catch (err) {
  ...
  const openEditDialog = (paper: FullLengthMock) => {
  ...
  const handleDragStart = (e: React.DragEvent, paper: FullLengthMock) => {
  ...
  const handleDragEnter = (paper: FullLengthMock) => {
  ...
  const handleDrop = (e: React.DragEvent, targetPaper: FullLengthMock) => {
    e.preventDefault();
    if (!isReorderingEnabled) return;
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, examFilter, pageSize]);
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Create a new full-length mock test for complete exam simulation.
                  </DialogDescription>
                </DialogHeader>
                {renderForm()}
                <DialogFooter>
                  ...
                  <Button onClick={handleAdd} disabled={addSaving}>
                    {addSaving ? "Saving…" : "Add Mock Test"}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div className="space-y-1">
                    ...
                    <CardDescription className="text-sm">
                      Manage full-length mock tests for complete exam simulation
                    </CardDescription>
                  </div>
                  <div className="flex items-center space-x-2">
                    {!isReorderingEnabled ? (
                    ...
                    <TableBody>
                      {pagedPapers.map((paper) => (
                        ...
                        >
                          ...
                          >
                            <Badge variant="outline">{paper.examName}</Badge>
                          </TableCell>
                          <TableCell
            ...
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
              ...
              <DialogFooter>
                ...
                <Button onClick={handleEdit} disabled={editSaving}>
                  {editSaving ? "Saving…" : "Update Mock Test"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
            <DialogContent className="sm:max-w-[425px]">
              ...
              <DialogFooter>
                ...
                >
                  {deleteSaving ? "Deleting…" : "Delete Test"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
