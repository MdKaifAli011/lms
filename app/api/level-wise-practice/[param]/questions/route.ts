...
export async function GET(
  ...
  try {
    ...
    const practiceId = (practice._id as mongoose.Types.ObjectId).toString();

    const questions = await LevelWiseQuestion.find({ practiceId: practice._id })
      .sort({ orderNumber: 1 })
      .lean();
    ...
    console.error("GET /api/level-wise-practice/[param]/questions error:", err);
...
/** POST /api/level-wise-practice/[param]/questions – create a question */
export async function POST(
  ...
  try {
    ...
    const maxOrder = await LevelWiseQuestion.findOne({ practiceId: practice._id })
      .sort({ orderNumber: -1 })
      .select("orderNumber")
      .lean();
    const orderNumber = ((maxOrder?.orderNumber as number) ?? 0) + 1;

    const doc = await LevelWiseQuestion.create({
    ...
    console.error("POST /api/level-wise-practice/[param]/questions error:", err);
    return NextResponse.json(
