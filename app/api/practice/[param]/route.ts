...
/** GET /api/practice/[param] – get a single practice paper by ID or slug */
export async function GET(
  ...
  try {
    ...
    const doc = await PracticePaper.findOne(query)
...
/** PUT /api/practice/[param] – update a practice paper */
export async function PUT(
  ...
  try {
    ...
    const existingDoc = await PracticePaper.findOne(query).lean();
    if (!existingDoc) {
      return NextResponse.json({ error: "Practice paper not found" }, { status: 404 });
    }
    ...
    if (body.title !== undefined) {
      ...
      if (title !== currentTitle) {
        ...
        let slug = baseSlug;
        let counter = 1;
        while (await PracticePaper.findOne({ examId, slug, _id: { $ne: (existingDoc as { _id: mongoose.Types.ObjectId })._id } }).lean()) {
    ...
    const doc = await PracticePaper.findOneAndUpdate(
      query,
      { $set: updateData },
      { new: true }
    )
...
/** DELETE /api/practice/[param] – delete a practice paper */
export async function DELETE(
  ...
  try {
    ...
    const doc = await PracticePaper.findOneAndDelete(query).lean();
