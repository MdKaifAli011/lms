...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ param: string }> }) {
  try {
    ...
    await connectDB()
    const doc = await Chapter.findById(param).select("slug unitId seo").lean()
    ...
    return NextResponse.json({
      ...
      unitId: (doc as { unitId?: { toString: () => string } }).unitId?.toString(),
    ...
    console.error("GET /api/chapters/[param]/meta error:", err)
