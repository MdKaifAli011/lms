...
/** POST /api/subtopics/reorder - batch update orderNumber for subtopics */
export async function POST(request: NextRequest) {
  try {
    ...
    for (const item of updates as { id: string; orderNumber: number }[]) {
      await Subtopic.findByIdAndUpdate(item.id, { orderNumber: item.orderNumber, lastModified, updatedAt: new Date() })
    }
    return NextResponse.json({ ok: true })
  } catch (err) {
    console.error("POST /api/subtopics/reorder error:", err)
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to reorder subtopics" },
      { status: 500 }
    )
  }
}
