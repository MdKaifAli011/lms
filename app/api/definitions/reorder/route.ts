...
/** POST /api/definitions/reorder - batch update orderNumber for definitions */
export async function POST(request: NextRequest) {
  try {
    ...
    for (const item of updates as { id: string; orderNumber: number }[]) {
      await Definition.findByIdAndUpdate(item.id, { orderNumber: item.orderNumber, lastModified, updatedAt: new Date() })
    }
    return NextResponse.json({ ok: true })
  } catch (err) {
    console.error("POST /api/definitions/reorder error:", err)
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to reorder definitions" },
      { status: 500 }
    )
  }
}
