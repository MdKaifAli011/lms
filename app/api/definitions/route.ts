...
export async function GET(request: NextRequest) {
  try {
    ...
    const definitions = await (contextapi ? query.select("name slug status orderNumber subtopicId weightage marks") : query).lean()

    if (contextapi) {
      type MinimalDoc = { _id: { toString(): string }; subtopicId: { toString(): string }; name?: string; slug?: string; status?: string; orderNumber?: number; weightage?: number; marks?: number }
