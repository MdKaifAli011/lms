...
| **Bold / Italic / Underline / Strikethrough** | `@lexical/selection`  | Format toggles                                        |
...
| **Mark (highlight)**                          | `@lexical/mark`       | Decorator nodes for highlights/keywords               |
...
| **Horizontal rule** | `@lexical/react`           | Divider line                                       |
...
| **Images**                    | ImagesPlugin                              | Upload, URL, resize, caption, alignment                                                                        |
| **Image row (2 col / 3 col)** | ImagesPlugin + ImageRowNode               | Insert Image → “One row 2 col” or “One row 3 col”; responsive grid with border/grid lines, persists after save |
| **YouTube**                   | YouTubePlugin                             | Embed YouTube videos                                                                                           |
| **Twitter/X**                 | TwitterPlugin                             | Embed tweets                                                                                                   |
| **Figma**                     | FigmaPlugin                               | Embed Figma frames                                                                                             |
| **Excalidraw**                | ExcalidrawPlugin + @excalidraw/excalidraw | In-editor whiteboard/diagrams                                                                                  |
| **Polls**                     | PollPlugin                                | Inline poll blocks                                                                                             |
| **Sticky notes**              | StickyPlugin                              | Draggable sticky notes                                                                                         |
| **Date/Time**                 | DateTimePlugin                            | Date/time picker block                                                                                         |
...
| **Mentions**               | MentionsPlugin                  | @mention typeahead               |
| **Emojis**                 | EmojisPlugin, EmojiPickerPlugin | Emoji picker and shortcuts       |
| **Equations**              | EquationsPlugin + katex         | LaTeX math                       |
| **Markdown shortcuts**     | `@lexical/markdown`             | Type #, ##, -, 1., [] for blocks |
| **Markdown import/export** | `@lexical/markdown`             | Paste/export Markdown            |
| **HTML import/export**     | `@lexical/html`                 | Paste/export HTML                |
| **Drag & drop paste**      | DragDropPastePlugin             | Drop images/files into editor    |
...
| **Floating format bar**  | FloatingTextFormatToolbarPlugin                  | Bold/italic on text selection           |
...
| Package              | Purpose                                                 |
| -------------------- | ------------------------------------------------------- |
| `@lexical/extension` | Extension API (defineExtension, DecoratorTextExtension) |
| `@lexical/headless`  | Headless editor (no DOM) – e.g. server validation       |
| `@lexical/overflow`  | Overflow/scroll behavior                                |
| `@lexical/utils`     | Helpers: mergeRegister, $insertNodeToNearestRoot, etc.  |

---

## Optional (not in playground)

| Package             | Feature                            |
| ------------------- | ---------------------------------- |
| `@lexical/devtools` | Lexical DevTools panel in browser  |
| `@lexical/history`  | Standalone history (undo/redo)     |
...
- **Your setup:** Full Lexical Playground in `editor/` plus all required `@lexical/*` packages and `@lexical/yjs` for collaboration and commenting.
- **You have:** Every feature above (toolbar, insert menu, tables, images, layouts, equations, Excalidraw, stickies, links, lists, code, markdown, hashtags, mentions, emojis, collab, comments, floating toolbars, etc.).
- **To use it:** Open the page that mounts the playground (e.g. `/playground`). No need to change any Lexical or playground code.
