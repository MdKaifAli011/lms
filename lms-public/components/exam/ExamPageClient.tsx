...
export function ExamPageClient({ exams }: ExamPageClientProps) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {/* Hero — overflow-hidden so blur orbs don't extend section and cause scrollbar */}
      <GradientBg variant="subtle" intensity="low" className="relative overflow-hidden">
        <div
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full min-w-0 px-3 min-[480px]:px-4 sm:px-5 md:px-6 pt-10 sm:pt-12 md:pt-16 lg:pt-20 pb-10 sm:pb-12 md:pb-16 lg:pb-20">
          <div className="text-center space-y-6 sm:space-y-8 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
              {[
                ...
                { value: stats.totalStudents, label: "Students", icon: Users, color: "bg-blue-500/10 text-blue-600 dark:text-blue-400" },
                { value: stats.avgRating, label: "Avg rating", icon: Star, color: "bg-amber-500/10 text-amber-600 dark:text-amber-400" },
                { value: stats.successRate, label: "Success rate", icon: Award, color: "bg-violet-500/10 text-violet-600 dark:text-violet-400" },
              ].map(({ value, label, icon: Icon, color }) => (
                <Card
                  key={label}
                  className="bg-card/80 dark:bg-card/60 backdrop-blur-xl border border-border shadow-lg transition-all hover:shadow-xl hover:border-primary/20"
                >
            ...
            <div className="flex flex-col min-[480px]:flex-row gap-3 justify-center pt-1 sm:pt-2">
              <Button size="default" className="gap-2 h-10 sm:h-11 px-5 sm:px-6 rounded-xl font-semibold text-sm sm:text-base" asChild>
                <Link href="#programs">
                  <Target className="h-4 w-4" />
                  Explore programs
                </Link>
              </Button>
              <Button size="default" variant="outline" className="gap-2 h-10 sm:h-11 px-5 sm:px-6 rounded-xl font-semibold text-sm sm:text-base border-2" asChild>
                <Link href="/">
      ...
      {/* Mastery path — 7-level framework */}
      <MasteryPathSection />
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 items-center">
                <select
                  ...
                  className="h-10 sm:h-11 min-w-[180px] rounded-xl border border-border bg-background px-3 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                >
              ...
              {filteredAndSortedExams.map((exam) => (
                <div key={exam.id} className="min-w-0 transition-transform duration-200 hover:-translate-y-0.5">
                  <ExamCard
                    exam={{
                      id: exam.id,
                      name: exam.name ?? exam.slug ?? exam.id,
                      slug: exam.slug,
                      image: exam.image,
        ...
        <div className="max-w-7xl mx-auto w-full min-w-0 px-3 min-[480px]:px-4 sm:px-5 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
            {[
              ...
              <Card
                key={name}
                className="bg-card/80 dark:bg-card/60 backdrop-blur-xl border border-border shadow-lg transition-all hover:shadow-xl hover:border-primary/20 overflow-hidden"
              >
      ...
      <section className="py-10 sm:py-14 md:py-16 lg:py-20 px-3 min-[480px]:px-4 sm:px-5 md:px-6">
        <div className="max-w-7xl mx-auto w-full min-w-0">
          <GradientBg variant="subtle" intensity="low" className="relative overflow-hidden rounded-xl sm:rounded-2xl border border-border">
            <div
              className="absolute inset-0 -z-10 bg-[radial-gradient(hsl(var(--muted))_1px,transparent_1px)] [background-size:18px_18px] sm:[background-size:22px_22px]"
              aria-hidden
            />
