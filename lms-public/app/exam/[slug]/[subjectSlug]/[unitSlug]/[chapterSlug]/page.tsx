...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!chapter || typeof chapter !== "object") return { title: "Not Found | LmsDoors" };
  const chapterName = toTitleCase(String((chapter as { name?: string }).name ?? chapterSlug));
  const seo = normalizeApiSeo((chapter as { seo?: unknown }).seo);
...
export default async function ChapterPage({ params }: PageProps) {
  ...
  ).map((t) => ({ id: t.id, href: `/exam/${examSlug}/${subjectSlug}/${unitSlug}/${chapterSlug}/${t.slug ?? t.id}`, slug: t.slug, name: toTitleCase(t.name ?? "") }));
