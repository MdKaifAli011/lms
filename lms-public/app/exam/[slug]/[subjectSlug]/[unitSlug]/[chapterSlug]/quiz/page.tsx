...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: `Quiz – ${chapterName} | LmsDoors` };
}
...
export default async function ChapterQuizPage({ params }: PageProps) {
  ...
  const breadcrumbs = [
    ...
    { label: "Quiz", href: `/exam/${examSlug}/${subjectSlug}/${unitSlug}/${chapterSlug}/quiz` },
  ];

  const prevHref = `/exam/${examSlug}/${subjectSlug}/${unitSlug}/quiz`;
  const nextHref = firstTopic
    ? `/exam/${examSlug}/${subjectSlug}/${unitSlug}/${chapterSlug}/${firstTopic.slug ?? firstTopic.id}/quiz`
    : null;
  ...
  return (
    <>
      ...
      <LevelQuiz
        ...
        title={`Practice quiz – ${chapterName}`}
      />
