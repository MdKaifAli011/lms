...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: `Quiz – ${unitName} | LmsDoors` };
}
...
export default async function UnitQuizPage({ params }: PageProps) {
  ...
  const breadcrumbs = [
    ...
    { label: "Quiz", href: `/exam/${examSlug}/${subjectSlug}/${unitSlug}/quiz` },
  ];

  const prevHref = `/exam/${examSlug}/${subjectSlug}/quiz`;
  const nextHref = firstChapter
    ? `/exam/${examSlug}/${subjectSlug}/${unitSlug}/${firstChapter.slug ?? firstChapter.id}/quiz`
    : null;
