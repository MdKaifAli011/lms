...
/** Seven hierarchy levels: each level has its own color for text and indicators */
type HierarchyLevel = 'subject' | 'unit' | 'chapter' | 'topic' | 'subtopic' | 'definition' | 'exam'

const LEVEL_COLORS: Record<
  HierarchyLevel,
  { text: string; textActive: string; bg: string; bgHover: string; border: string; dot: string; accent: string }
> = {
  exam: {
    text: 'text-slate-600 dark:text-slate-400',
    textActive: 'text-slate-900 dark:text-slate-100',
    bg: 'bg-slate-100/80 dark:bg-slate-800/40',
    bgHover: 'hover:bg-slate-100 dark:hover:bg-slate-800/60',
    border: 'border-slate-300 dark:border-slate-600',
    dot: 'bg-slate-500 dark:bg-slate-400',
    accent: 'bg-slate-500/10 dark:bg-slate-400/10',
  },
  subject: {
  ...
  subtopic: {
    ...
    textActive: 'text-rose-900 dark:text-rose-100',
  ...
  definition: {
    ...
    textActive: 'text-cyan-900 dark:text-cyan-100',
    bg: 'bg-cyan-50/80 dark:bg-cyan-950/30',
    bgHover: 'hover:bg-cyan-50 dark:hover:bg-cyan-950/50',
    border: 'border-cyan-300 dark:border-cyan-700',
    dot: 'bg-cyan-600 dark:bg-cyan-400',
    accent: 'bg-cyan-500/10 dark:bg-cyan-400/10',
  },
}

interface SubjectWithUnits {
  id: string
  slug?: string
  title?: string
...
export function HierarchySidebar({
  ...
  const visibleSubjects = useMemo(() => {
    ...
    return orderedSubjects.filter((s) =>
      JSON.stringify(s).toLowerCase().includes(searchQuery.toLowerCase()),
    )
    ...
    <>
      ...
      <aside
        className={cn(
          ...
          'scrollbar-thin scrollbar-track-transparent scrollbar-thumb-muted-foreground/20 scrollbar-thumb-rounded-full hover:scrollbar-thumb-muted-foreground/40 transition-colors',
          isMobile
            ? 'fixed left-0 top-[80px] sm:top-[96px] h-[calc(100vh-80px)] sm:h-[calc(100vh-96px)] z-50 w-80 animate-in slide-in-from-left duration-300'
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          <div className="relative group">
            ...
            <Input
              ...
              className="h-10 pl-9 pr-9 text-sm bg-muted/50 border-border text-foreground placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:border-primary/50 transition-all duration-200"
            />
            {searchQuery && (
              <button
              ...
              visibleSubjects.map((subject) => {
                ...
                return (
                  <div key={subject.id} className="space-y-1">
                    ...
                    {expanded(subjectId, subjectActive) && subject.units && subject.units.length > 0 && (
                      <div className="relative ml-3 pl-4 border-l-2 border-dashed border-border/60 space-y-1 mt-1">
                        {subject.units.map((unit: UnitWithChapters) => {
                          ...
                          return (
                            <div key={unit.id} className="space-y-1">
                              ...
                              {expanded(unitId, unitActive) && unit.chapters && unit.chapters.length > 0 && (
                                <div className="relative ml-3 pl-4 border-l-2 border-dashed border-border/60 space-y-1 mt-1">
                                  {unit.chapters.map((chapter: ChapterWithTopics) => {
...
/* ---------------- BRANCH ROW (Subject / Unit / Chapter) ---------------- */
function BranchRow({
  level,
  href,
  label,
  ...
  return (
    ...
    >
      ...
      {hasLeadingCircle && (
        <div
          className={cn(
            'shrink-0 w-3 h-3 rounded-full border-2 bg-background transition-all duration-200',
            colors.border,
      ...
      {onToggle && (
        <button
          ...
          className={cn(
            'shrink-0 w-7 h-7 rounded-md flex items-center justify-center transition-all duration-200',
...
function TopicRow({
  ...
  return (
    <div className={cn('relative flex items-center py-1.5 min-h-[40px] -ml-4 pl-3', isLast && 'pb-2')}>
      ...
      <div
        className={cn(
          'relative z-[1] shrink-0 w-2.5 h-2.5 rounded-full transition-all duration-200',
          colors.dot,
          active && 'scale-125 shadow-sm ring-2 ring-background',
        )}
        aria-hidden
      />
      ...
      <Link
        ...
        title={label}
        className={cn(
          'flex-1 min-w-0 py-2 pl-3 pr-2 rounded-md transition-all duration-200 truncate text-sm font-normal group',
          active ? colors.textActive : colors.text,
      ...
      >
        <span className="relative">
          {label}
          {active && (
            <span className={cn('absolute -left-1.5 top-1/2 -translate-y-1/2 w-1 h-1 rounded-full', colors.dot)} aria-hidden />
          )}
        </span>
      </Link>
    </div>
