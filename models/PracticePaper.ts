...
/**
 * Practice paper type:
 * - practice = Level-wise practice (scope: exam → subject → unit → chapter → topic → subtopic → definition)
 * - full_length = Full-length mock (exam-level, complete paper)
 * - previous_paper = Previous year paper (exam-level, with year)
 */
...
export interface IPracticePaper {
  ...
  /** Content hierarchy level: 1=Exam, 2=Subject, ..., 7=Definition. For level-wise (type=practice) only. */
  level: ContentLevel;
  /** Optional refs for level 2–7 (scope of the practice). Required when type=practice and level >= corresponding level. */
...
/** Level-wise listing: filter by type=practice and level, sort by level then order */
practicePaperSchema.index({ type: 1, level: 1, orderNumber: 1 });
practicePaperSchema.index({ type: 1, examId: 1, level: 1, orderNumber: 1 });
